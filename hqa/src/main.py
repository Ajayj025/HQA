...
from utils.translator import translate_text  # NEW IMPORT

# Must be the first Streamlit command
st.set_page_config(
    page_title="HIA - Heal Quick Agent",
    page_icon="⚕️",
    layout="wide"
)
...
def show_chat_history():
    # Always show the input so user can change language anytime
    target_lang = st.text_input("Preferred language (e.g., en, hi, fr, te, ta, kn):", 
                                value=st.session_state.get("target_language", "en"))
    
    # Update session state every run
    st.session_state.target_language = target_lang.strip()
    ...
    if success:
        for msg in messages:
            if msg['role'] == 'user':
                ...
                # Translate only non-user messages
                translated_content = translate_text(msg['content'], target_language=target_lang)
